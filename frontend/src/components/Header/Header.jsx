...
const NavLinks = [
  ...
  { key: 'about-us', display: 'About Us' }, // About Us added here
];

const Header = ({ onScrollToSection, hideStudyPlanButton = false, alwaysWhite = false }) => {
  ...
  useEffect(() => {
    if (alwaysWhite) {
      // Skip scroll listener if alwaysWhite is true
      return;
    }
    const handleScroll = () => {
  ...
  const isHeaderWhite = alwaysWhite || isScrolled;
  ...
  const handleNavClick = (key) => {
    if (key === 'about-us') {
      ...
      if (location.pathname !== '/') {
        // Navigate home and pass scroll instruction in state
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${
        isHeaderWhite ? 'bg-white shadow-lg text-black' : 'bg-transparent text-white'
      }`}
    >
      <div className="container mx-auto flex items-center justify-between h-[100px] px-4">
        ...
        <div className="w-[160px]">
          <img
            src={isHeaderWhite ? scrolledLogo : logo}
            alt="Logo"
